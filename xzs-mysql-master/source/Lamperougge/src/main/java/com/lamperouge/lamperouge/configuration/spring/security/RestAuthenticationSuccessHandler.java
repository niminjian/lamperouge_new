...
public class RestAuthenticationSuccessHandler extends SimpleUrlAuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) {
        ...
        if (null != object) {
            ...
            com.lamperouge.lamperouge.domain.User user = userService.getUserByUserName(springUser.getUsername());
            if (null != user) {
                UserEventLog userEventLog = new UserEventLog(user.getId(), user.getUserName(), user.getRealName(), new Date());
                userEventLog.setContent(user.getUserName() + " 登录了Lamperouge教考分离系统");
                eventPublisher.publishEvent(new UserEvent(userEventLog));
                com.lamperouge.lamperouge.domain.User newUser = new com.lamperouge.lamperouge.domain.User();
